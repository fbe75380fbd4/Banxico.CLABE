...
Copy-Item -Path "$($WorkingDirectory)\Banxico.CLABE" -Destination $publishDir.FullName -Recurse -Force

#region Gather text data to compile
$text = @()

# Gather commands
Get-ChildItem -Path "$($publishDir.FullName)\Banxico.CLABE\internal\functions\" -Recurse -File -Filter "*.ps1" | ForEach-Object {
	$text += [System.IO.File]::ReadAllText($_.FullName)
}
Get-ChildItem -Path "$($publishDir.FullName)\Banxico.CLABE\functions\" -Recurse -File -Filter "*.ps1" | ForEach-Object {
	$text += [System.IO.File]::ReadAllText($_.FullName)
}
...
if ($LocalRepo)
{
	...
	Write-Host  "Creating Nuget Package for module: Banxico.CLABE"
	New-PSMDModuleNugetPackage -ModulePath "$($publishDir.FullName)\Banxico.CLABE" -PackagePath .
}
else
{
	# Publish to Gallery
	Write-Host  "Publishing the Banxico.CLABE module to $($Repository)"
	Publish-Module -Path "$($publishDir.FullName)\Banxico.CLABE" -NuGetApiKey $ApiKey -Force -Repository $Repository
}
#endregion Publish